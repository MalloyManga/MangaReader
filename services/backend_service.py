        ...
        try:
            ...
            # 4. 提取封面 (New)
                ...
                try:
                    ...
                    if cover_base64:
                        send_response({"id": req_id, "success": True, "cover": cover_base64})
                    else:
                        send_response({"id": req_id, "success": False, "error": "No image found"})
                except Exception as e:
                    ...
                    if file_name.lower().endswith(allowed_exts):
                        try:
                            with zip_ref.open(file_name) as file:
                             ...
                             log_message(f"Failed to read zip entry {file_name}: {img_err}")
                             continue
